...
function Calendar(element, options, eventSources) {
	...
	function renderView(inc) {
		if (elementVisible()) {
			...
			var hoje = new Date();
			if (hoje >= currentView.start && hoje < currentView.end) {
	...
	function renderBasic(r, c, showNumbers) {
		...
		if (!tbody) { // first time, build all cells from scratch
			...
			tbody.find('td').each(function() {
				...
				if (+d == +hoje) {
					td.removeClass('fc-not-hoje')
						.addClass('fc-hoje')
						.addClass(tm + '-state-highlight');
				}else{
					td.addClass('fc-not-hoje')
						.removeClass('fc-hoje')
		...
		if (!head) { // first time rendering, build from scratch
			...
			for (i=0; i<colCnt; i++) {
				s += "<td class='fc-" +
					...
					tm + '-state-default ' +
					(!i ? 'fc-leftmost ' : '') +
					(+d == +hoje ? tm + '-state-highlight fc-hoje' : 'fc-not-hoje') +
			...
			bg.find('td').each(function(i, td) {
				...
				if (+d == +hoje) {
					$(td)
						.removeClass('fc-not-hoje')
						.addClass('fc-hoje')
						.addClass(tm + '-state-highlight');
				}else{
					$(td)
						.addClass('fc-not-hoje')
						.removeClass('fc-hoje')
						.removeClass(tm + '-state-highlight');
				}
				addDays(d, dis);
